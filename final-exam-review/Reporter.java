...
public class Reporter implements Visitor { // 1
    ...
    String lLine(Node node) {
        if (node.color == RedBlackTree.RED) { // red (0)
            return "[" + node.data + "]";
        } else if (node.color == 2) { // double black (2)
            return "(" + node.data + ")";
        } else if (node.color == -1) { // negative red (-1)
            return "<" + node.data + ">";
        } else { // black (1)
            return "" + node.data + "";
        }
    ...
    public Displaux displaux(Node node) { // 2
        if (node.right == null && node.left == null) { // 3
            // String line = (node.color == RedBlackTree.RED) ? "[" + node.data + "]" : node.data + ""; // [1]
            String line = lLine(node);
        ...
        if (node.right == null) { // 3
            ...
            int p = result.height;
            int x = result.middle;
            // String s = (node.color == RedBlackTree.RED) ? "[" + node.data + "]" : node.data + ""; // [2]
            String s = lLine(node);
            int u = s.length();
        ...
        if (node.left == null) { // 3
            ...
            int p = result.height;
            int x = result.middle;
            // String s = (node.color == RedBlackTree.RED) ? "[" + node.data + "]" : node.data + ""; // [3]
            String s = lLine(node);
            int u = s.length();
        ...
        int q = result.height;
        int y = result.middle;
        // String s = (node.color == RedBlackTree.RED) ? "[" + node.data + "]" : node.data + ""; // [4]
        String s = lLine(node);
        int u = s.length();
